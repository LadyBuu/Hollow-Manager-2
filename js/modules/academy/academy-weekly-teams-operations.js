 ...
 *
 * RANDOMIZATION:
 *   The unassigned pool is shuffled ONCE, before the fill loop,
 *   using the Fisher-Yates algorithm. Every run over the same
 *   input produces a different team composition, because the pool
 *   order determines who lands in which team.
 *
 *   WHY SHUFFLE THE POOL:
 *     The fill loop is deterministic given the pool order: it
 *     walks the pool front-to-back and places each student into
 *     the least-full team with room. If the pool arrives
 *     alphabetized (as the aggregator's roster does), every run
 *     places the same students into the same teams. Shuffling the
 *     pool breaks that correlation without changing the fill
 *     policy.
 *
 *   WHY NOT SHUFFLE AFTER FILL:
 *     The fill's least-full preference is what keeps teams balanced.
 *     Shuffling the pool before the fill preserves that balance and
 *     only varies WHO lands in WHICH team. Shuffling after fill
 *     would break team size balance.
 *
 *   DETERMINISM WITHIN A RUN:
 *     The shuffle uses Math.random(), which is not seeded. Two runs
 *     on the same data produce different results by design. If a
 *     future caller needs reproducible distribution, it should pass
 *     a seed; the module does not expose one today.
 *
 ...
 *   the shuffle means the second run's team composition differs
 *   from the first. The clearExisting flag exists so a user can
 *   wipe the week's assignments and re-run.
 *
 * FAILURE SEMANTICS:
...
(function() {
    ...
    // ============================================================
    //
    // Fisher-Yates, in-place. Every permutation is equally likely.
    //
    ...
    //
    // Do NOT replace this with arr.sort(() => Math.random() - 0.5).
    // That comparator is not a valid sort comparator (it is not
    // transitive) and produces a biased shuffle. Fisher-Yates is
    ...
    function runAutoDistributeCore(ctx, groupSize, week, deps) {
        ...
        // ---- Unassigned pool ----
        //
        // The aggregator returns its roster alphabetically. We
        // filter that roster to the unassigned students, then
        // SHUFFLE the result. The shuffle is what makes each run
        // produce a different team composition.
        var unassigned = [];
        ...
        //      supports a full one. ----
        //
        // The remainder is the students Phase 1 could not place.
        // A student is "placed by Phase 1" when they appear in
        // some existingPlans[].charIds.
