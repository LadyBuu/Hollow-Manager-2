 ...
 *     least-full existing team with room below groupSize. Students
 *     who cannot be placed (because every existing team is already
 *     at groupSize) are collected into the remainder.
 *
 ...
 *     from the front of the remainder. A partial team is never
...
(function() {
    ...
    function runAutoDistributeCore(ctx, groupSize, week, deps) {
        ...
        //
        // We build the existing plans first, then walk the
        // unassigned pool. A student is placed when some existing
        // team has room below groupSize. A student who cannot be
        // placed is deferred to the remainder — we do NOT break the
        // walk, because the pool's later students also need to be
        // considered (they might have been intended for an
        // existing team that has since filled; deferring is the
        // correct outcome for them too).
        var existingPlans = [];
        ...
        var placedCharIds = Object.create(null);
        var remainder = [];
        ...
        for (var u = 0; u < unassigned.length; u++) {
            ...
            if (bestPlan) {
                ...
                placedCharIds[studentId] = true;
            } else {
                remainder.push(unassigned[u]);
            }
        }
        ...
        // While at least groupSize of them remain, carve off a
        // full team. This is the ONLY place a new team is created.
        // A new team is filled to groupSize from the front of the
        // remainder, in the order the students were sorted at the
        // top of the function.
        var newPlans = [];
        ...
        while (remainder.length >= groupSize) {
            ...
            for (var f = 0; f < groupSize; f++) {
                var frontStudent = remainder.shift();
                newPlan.charIds.push(String(frontStudent.id));
            }
        ...
        //
        // At this point every plan (existing + newly created) is at
        // exactly groupSize. The "ascending order" is the stable
        // order the plans were built in: existing plans first
        // (ranked by pre-fill count), then new plans in creation
        // order. Cycling index % allPlans.length walks that order
        // repeatedly.
        ...
        if (remainder.length > 0) {
            ...
            if (allPlans.length === 0) {
                ...
                // groupSize. The pinboard rule says "a new team is
                // only created when R >= N" and "a partial team is
                // never created." Both forbid creating a team here.
                //
                // This can happen only when there are zero existing
                // active teams AND fewer than groupSize unassigned
                // students. The correct resolution is out of scope
                // for this file; we surface it rather than silently
                // swallowing the students.
