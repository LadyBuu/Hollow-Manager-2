 ...
 * Works like student calendar with the same data structure
 * Groups are pulled from the discipline groups system (groups.js)
...
(function() {
    ...
    function renderGroupList() {
        ...
        if (groupKeys.length === 0) {
            html += '<div style="text-align:center;color:var(--text-dim);font-size:0.8rem;padding:8px;">No groups found for this instructor. Create groups in the Groups tab.</div>';
        } else {
    ...
    function renderInstructorCalendarData() {
        ...
        var templates = window.getInstructorTemplatesForWeek ? window.getInstructorTemplatesForWeek(instructorId, week) : {};
        ...
        dayColumns.forEach(function(column, index) {
            ...
            hours.forEach(function(hour) {
                ...
                if (slotData) {
                    ...
                    if (activeFilter !== 'all' && slotData.groupLabel !== filterGroupLabel && !slotData.isTemplate) {
    ...
    function showClassManagementModal(slotData, day, hour) {
                ...
                if (typeof window.saveData === 'function') {
                    window.saveData().then(function() {
                        if (typeof window.logActivity === 'function') {
                            window.logActivity('Added ' + toAdd.length + ' students from group ' + selectedGroup + ' to ' + discipline.name);
                        }
                        modal.remove();
                        renderInstructorCalendarData();
                        populateGroupFilter();
        ...
        modal.querySelector('#assign-students-btn').onclick = function() {
            ...
            if (typeof window.saveData === 'function') {
                window.saveData().then(function() {
                    if (typeof window.logActivity === 'function') {
                        window.logActivity('Updated student assignments for ' + discipline.name +
                            (label ? ' [' + label + ']' : '') +
    ...
    function removeInstructorClass(slotData, day, hour) {
        ...
        if (typeof window.saveData === 'function') {
            window.saveData().then(function() {
                if (typeof window.logActivity === 'function') {
                    var discipline = window.getDiscipline(disciplineId);
                    window.logActivity('Removed class ' + (discipline ? discipline.name : '') + ' from ' + studentIds.length + ' student(s)');
                }
                renderInstructorCalendarData();
                populateGroupFilter();
                if (typeof window.renderStudentSchedule === 'function') {
        ...
        modal.querySelector('#confirm-add-class').onclick = function() {
            ...
            if (typeof window.saveData === 'function') {
                window.saveData().then(function() {
                    if (typeof window.logActivity === 'function') {
                        var discipline = window.getDiscipline(disciplineId);
                        window.logActivity('Added template class ' + (discipline ? discipline.name : '') +
                            (label ? ' [' + label + ']' : '') +
                            (groupLabel ? ' (Group ' + groupLabel + ')' : '') +
    ...
    function showAddBlockModal() {
        ...
        modal.querySelector('#confirm-block').onclick = function() {
            ...
            if (typeof window.saveData === 'function') {
                window.saveData().then(function() {
                    if (typeof window.logActivity === 'function') {
                        window.logActivity('Blocked time for instructor ' + instructorName + ': ' + label +
                            (groupLabel ? ' (Group ' + groupLabel + ')' : '') +
                            ' (' + duration + 'h)' +
    ...
    function removeBlockedTime(day, hour) {
        ...
        if (typeof window.saveData === 'function') {
            window.saveData().then(function() {
                if (typeof window.logActivity === 'function') {
                    window.logActivity('Removed blocked time');
                }
                renderInstructorCalendarData();
                populateGroupFilter();
            }).catch(function(err) {
