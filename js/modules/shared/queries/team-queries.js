 ...
 *   getActiveTeamMembers checks the team's OWN window
 ...
 *   from the returned list, never add them.
 *
 * MEMBER IDENTITY (v25):
 *   [FIX-Q2] Every member entry can carry a stable `memberId`.
 *   Entries that predate the memberId introduction fall back to a
 *   composite key of (characterId, joinPeriod).
 *
 *   The reason: a character may have multiple stints on the same
 *   team (they left, they came back). Each stint is a distinct
 *   entry in members[]. Without a stable per-entry identifier,
 *   "edit this stint's leave week" is ambiguous when the same
 *   characterId appears twice.
 *
 *   New lookups:
 ...
 *
 *   Existing lookups keep their shapes:
 *     getTeamMember(team, characterId) — returns the FIRST matching
 *     entry. Callers that need a specific stint use the new
 *     lookups.
 *
 *   memberId is not generated by this module. It is generated by
 *   the mutation layer on first write. Reads pass through whatever
 *   the entry carries, including absence.
 *
 * DEPENDENCIES:
...
(function() {
     ...
     * "characterId::joinPeriod". This helper exists so that the
    ...
    function makeMemberIdentityKey(member) {
        ...
        var charId = member.characterId !== undefined &&
            ...
            : '';
        var join = member.joinPeriod !== undefined &&
                   member.joinPeriod !== null
            ? String(member.joinPeriod)
            : '';
        return 'composite:' + charId + '::' + join;
    }
     ...
     * record, so any memberId (or absence thereof) is preserved.
     *
     * @param {object} team
     * @param {number|string} period
    ...
    /**
     * [FIX-Q2a] Get a specific member entry by memberId. Returns a
     * clone, or null.
     *
     * memberId is generated lazily by the mutation layer. Entries
     * persisted before memberId existed return null here until they
     * are written once. Callers that need to address such entries
     * use getTeamMemberByComposite.
     */
    ...
    /**
     * [FIX-Q2b] Get a team's member record for a character, scoped
     * by stint. Identifies the entry by (characterId, joinPeriod).
     *
     * Used as a fallback when a memberId is not available (legacy
     * records persisted before memberId was introduced).
     *
     * `joinPeriod` may be '' for entries with no join bound. The
     * comparison is on the canonical string form of the stored
     * joinPeriod, not the parsed integer, so '' matches ''.
     */
    function getTeamMemberByComposite(team, characterId, joinPeriod) {
        ...
        for (var i = 0; i < team.members.length; i++) {
            ...
            var mJoin = (member.joinPeriod === undefined ||
                         member.joinPeriod === null)
                ? ''
                : String(member.joinPeriod);
            if (mJoin === targetJoin) {
    ...
    /**
     * [FIX-Q2c] Get all member entries for a team, current or former,
     * as a flat array of clones.
     *
     ...
     * member list.
     *
     * Each entry preserves its memberId (or absence) verbatim.
     */
