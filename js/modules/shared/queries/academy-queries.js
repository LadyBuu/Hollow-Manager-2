 ...
 *   - All roster reads in this module walk window.data.characters, not a
 *     separate roster. This means the character-side view and the
 *     academy-side view of "who is in class X" are always consistent by
...
(function() {
     ...
     * 
     * NOTE: This function USED to have a self-recursive body that would
     * stack-overflow when called. The version in this file is the correct
     * one. Do not reintroduce a second hoisted declaration of the same
    ...
    /**
     * Get the display name for a discipline.
     * 
     * NOTE: The original version of this function called itself
     * recursively. This version delegates to DisciplineQueries and
    ...
    // 
    // character.classIds is the single source of truth for membership.
    // These functions walk window.data.characters. There is no separate
    // roster store.
    // 
    // The functions are deliberately small and centralised so that any
    // future change to membership representation happens in one place.

    /**
     * Get the classIds array for a character, defensively normalised.
     * 
     * @param {object} char - Character object
     * @returns {string[]} Array of class IDs (never null)
     */
    ...
    /**
     * Check whether a character is a member of a class.
     * 
     * @param {object} char - Character object
    ...
    /**
     * Get all character IDs in a class, derived from character.classIds.
     * 
     * @param {string} classId - Class ID
     ...
     * Get all classes.
     * 
     * NOTE: Returns LIVE REFERENCES to the class objects, not deep
     * copies. This is deliberate for performance: the class list view
     * iterates this on every render. Callers that intend to mutate
     * should use getClass() (which deep-clones) instead, or go through
     * the AcademyClasses mutation API.
     * 
     ...
     * 
     * @param {string} classId - Class ID
     * @returns {object|null} Class object (deep clone) or null
     */
     ...
     * 
     * Reads from the character's own classIds. This is the canonical
     * relationship, so this query can never disagree with the derived
     * class roster.
     * 
     * @param {object} character - Character object
     * @returns {array} Array of class objects (deep clones)
     */
     ...
     * 
     * Reads directly from character.classIds. Delegation to
     * AcademyClasses is avoided here because the old implementation
     * read from a store that no longer exists; reading the character
     * directly is both simpler and guaranteed to be authoritative.
     * 
     * @param {object} character - Character object
     * @param {string} classId - Class ID
     ...
     * @returns {array} Array of character objects or character IDs
     */
     ...
     * Get a group summary.
     * 
     * @param {string} key - Group key     * @returns {object|null} Group summary or null
     */
    ...
    function getStudentDetails(studentId, week) {
        ...
        // Find ranking.
        //
        // NOTE: The original version read `student.classId` (a scalar
        // that does not exist on the character record). The character
        // record stores `classIds` as an array. This is now fixed by
        // iterating the character's classes and taking the first
        // ranking that matches.
        var ranking = null;
     ...
     * 
     * NOTE: The original version called itself recursively. This version
     * delegates to getDisciplineNameById(), which is a distinct function
     * name — avoiding the hoisting-shadow problem entirely.
     * 
     * @param {string} disciplineId - Discipline ID
