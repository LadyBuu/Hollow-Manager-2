 ...
 * Thin UI layer for student calendar
 * 
 * IMPORTANT:
...
(function() {
    ...
    function getStudentName(studentId) {
        return CharacterQueries.getCharacterNameById(studentId);
    }
    ...
    function bindEvents() {
        ...
        var saveBtn = _container.querySelector('#save-rest-days-btn');
        if (saveBtn) {
            var listener = UIBase.addEventListener(saveBtn, 'click', function() { handleSaveRestDays(); });
            if (listener) { _listeners.push(listener); }
        }
    }
    ...
    function bindEntitySelector() {
        ...
        UIBase.populateSelect(select, students, function(s) { return s.id; }, function(s) {
    ...
    function handleAddClass(day, hour) {
        ...
        var disciplines = window.DisciplineQueries.getAvailableDisciplines(_state.week);
        ...
        if (confirmBtn) {
            confirmBtn.onclick = function() {
                ...
                var result = ScheduleCore.setStudentSlot(_state.selectedId, _state.week, day, hour, disciplineId, duration);
    ...
    function handleClassDetails(day, hour) {
        ...
        modal.innerHTML = (
            '<div class="modal-content modal-detail-content">' +
                '<div class="modal-header">' +
                    '<h3>' + UIBase.escapeHtml(details.disciplineName) + '</h3>' +
                    '<button class="close-modal">&times;</button>' +
                '</div>' +
                '<div class="modal-body">' +
    ...
    function handleSaveRestDays() {
        ...
        var days = UIBase.getCheckboxValues(_container, '.rest-day-check');
