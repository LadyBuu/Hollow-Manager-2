...
(function() {
    ...
    function getLocationClassDetails(locationId, week, day, hour) {
        ...
        var disciplineId = schedule[dayNum][hourNum];
        var discipline = DisciplineQueries.getDiscipline(disciplineId);
        var metadata = getSlotMetadata(locationId, weekNum, dayNum, hourNum);
        ...
        return {
            ...
            instructorName: metadata && metadata.instructorId ? 
                CharacterQueries.getDisplayName(CharacterQueries.getCharacterById(metadata.instructorId)) : 'Not assigned',
            label: metadata ? metadata.label || '' : '',
            groupLabel: metadata ? metadata.groupLabel || '' : '',
    ...
    function getAvailableDisciplines(instructorId, week) {
        return getInstructorAvailableDisciplines(instructorId, week);
    }
...
})(); // <-- This is the missing closing brace that was causing the syntax error/**
 * js/modules/calendar/calendar-queries.js - Calendar Queries
 * Read-only query layer for calendar data
 * Path: js/modules/calendar/calendar-queries.js
 * 
