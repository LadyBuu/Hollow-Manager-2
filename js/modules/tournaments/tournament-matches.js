 ...
 * (diagnostic build — logs which return null fires)
 */
...
(function() {
    ...
    /**
     * DIAGNOSTIC BUILD: every `return null` is preceded by a
     * console.warn that names the failing branch.
     */
    function buildProposedMatch(base, updates, tournament, round, options) {
        ...
        function reject(reason) {
            console.warn('[TournamentMatches.buildProposedMatch] REJECT:', reason, {
                updates: updates,
                baseType: base && base.type,
                roundMatchSize: round && round.matchSize
            });
            return null;
        }
        ...
        if (!hasOnlyAllowedKeys(updates, allowedKeys)) {
            return reject('hasOnlyAllowedKeys');
        }
        ...
        if (updates.participants !== undefined) {
            ...
            if (participants === null) {
                return reject('normaliseIdArrayStrict(participants)');
            }
        ...
        if (updates.type !== undefined && updates.type !== base.type) {
            if (!isTypeChangeAllowed(base)) {
                return reject('isTypeChangeAllowed');
            }
            var validTypes = Constants.VALID_MATCH_TYPES;
            if (validTypes.indexOf(proposedType) === -1) {
                return reject('validTypes.indexOf(proposedType)');
            }
        }
        ...
        if (updates.status !== undefined &&
            ...
            if (validStatuses.indexOf(updates.status) === -1) {
                return reject('validStatuses.indexOf(updates.status)');
            }
            if (updates.status === 'completed' &&
                currentStatus !== 'completed') {
                if (!allowCompletion) {
                    return reject('allowCompletion === false');
                }
            }
        ...
        if (updates.isPairExam !== undefined) {
            if (proposedType !== 'group_exam') {
                return reject('isPairExam but not group_exam');
            }
        ...
        if (updates.results !== undefined) {
            if (proposedType !== 'group_exam') {
                return reject('results but not group_exam');
            }
            ...
            if (!resultCheck.valid) {
                return reject('validateResultMap(results): ' + resultCheck.message);
            }
        ...
        if (updates.teamResults !== undefined) {
            if (proposedType !== 'team_vs_team') {
                return reject('teamResults but not team_vs_team');
            }
            ...
            if (!teamCheck.valid) {
                return reject('validateResultMap(teamResults): ' + teamCheck.message);
            }
        ...
        if (updates.individualResults !== undefined) {
            if (proposedType !== 'team_vs_team') {
                return reject('individualResults but not team_vs_team');
            }
            ...
            if (!indCheck.valid) {
                return reject('validateResultMap(individualResults): ' + indCheck.message);
            }
        ...
        if (updates.pairings !== undefined) {
            ...
            if (proposedType !== 'group_exam' || !isPairExamFinal) {
                return reject('pairings but not pair exam');
            }
            var pairings = Schema.normalisePairings(updates.pairings);
            if (pairings === null) {
                return reject('normalisePairings');
            }
        ...
        if (matchSizeApplies(proposed.type)) {
            ...
            if (proposed.participants.length > 0 &&
                ...
                return reject('matchSizeApplies: length ' +
                    proposed.participants.length + ' !== ' + matchSize);
            }
        }
        ...
        for (var pi = 0; pi < proposed.participants.length; pi++) {
            ...
            if (Rules &&
                ...
                if (!Rules.isParticipantEligible(
                    ...
                    return reject('Rules.isParticipantEligible(' + pid + ')');
                }
            }
        }
        ...
        if (proposed.type === 'group_exam') {
            if (!validateMatchResultParticipants(
                ...
                return reject('validateMatchResultParticipants');
            }
        }
