 ...
 *   TournamentRules.validateResultMap, which skips the participant-
 *   membership check. The participant-membership check runs later in
 *   validateProposedMatch, against the actual round and participant
 *   list.
 *
 *   Before this revision, this file carried a local duplicate of
 *   validateResultMap. That duplicate is now gone; every call site
 *   delegates to TournamentRules.validateResultMap directly, with
 *   `requireAll` passed explicitly at each call.
 *
 * ELIMINATION CASCADE:
 ...
 *
 *   See the generateMatchesTolerant docstring for the full contract.
 *
 ...
 *         Moved to TournamentQueries. These are reads; the query
 *         module owns them. Callers that used
 *         TournamentMatches.getEligibleParticipants now call
 *         TournamentQueries.getEligibleParticipants.
 *
 *     - validateResultMap
 *         Duplicate of TournamentRules.validateResultMap. Every
 *         internal call site now delegates to Rules with an
 *         explicit `requireAll` value.
 *
 *     - getRoundMatches
 *         Pure pass-through to Queries.getMatches.
 *
 *     - getMatch
 *         Pure pass-through to Queries.getMatch.
 *
 *     - isMatchComplete
 *         Pure pass-through to Queries.isMatchComplete.
 *
 *     - getMatchAdvancing
 *         Pure pass-through to Queries.getMatchAdvancing.
 *
...
(function() {
    ...
    if (!Rules || typeof Rules.validateResultMap !== 'function') {
        _missing.push('TournamentRules.validateResultMap');
    }
    ...
    function buildProposedMatch(base, updates, tournament, round, options) {
        ...
        if (updates.results !== undefined) {
            ...
            var resultCheck = Rules.validateResultMap(
                updates.results,
                null,
                { requireAll: false }
            );
        ...
        if (updates.teamResults !== undefined) {
            ...
            var teamCheck = Rules.validateResultMap(
                updates.teamResults,
                null,
                { requireAll: false }
            );
        ...
        if (updates.individualResults !== undefined) {
            ...
            var indCheck = Rules.validateResultMap(
    ...
    // ============================================================
    //
    // Private helper. Called after every match mutation to keep the
    // round's derived status consistent with the match list.
    ...
    function completeMatch(tournamentId, roundId, matchId, result) {
        ...
        if (type === 'group_exam') {
            ...
            var participants = Array.isArray(match.participants)
                ? match.participants
                : [];
            var check = Rules.validateResultMap(
                rawResults,
                participants,
                { requireAll: true }
            );
    ...
    // ============================================================
    //
    // Private helpers. Called by generateMatches and
    // generateMatchesTolerant. Not exported.
    ...
    window.TournamentMatches = Object.freeze({
