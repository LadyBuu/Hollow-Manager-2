 ...
 *   - Missing loser is derived ONLY for completed standard 2-person
 *     matches when winner is present.
 *   - Conflicting loser → reject.
 *   - Final repaired object MUST pass structural/semantic validation
 *     via Schema.
 *
 * IDENTITY PRESERVATION (round and match IDs):
 *   Round and match records carry stable IDs generated by
 *   TournamentSchema (prefixes 'round' and 'match'). Repair MUST
 *   preserve these IDs when they are present:
 *
 *     - A round with a valid `id` keeps it.
 *     - A round with a missing or non-string `id` is rejected.
 *       Repair does not generate IDs. Generation is the schema's
 *       job (via normaliseTournament), and running repair before
 *       migration would silently rewrite identity. Any record that
 *       reaches repair without an ID is the result of a migration
 *       that hasn't run; the repair path rejects it and reports
 *       the reason so the operator can migrate first.
 *
 *     - Same rule applies to matches.
 *
 *   ID uniqueness is enforced within a round (no two matches share
 ...
 *     the stored value.
 *   - Numeric-string canonicalisation is allowed for week values.
 *   - addedAt is preserved as-is if present (schema-validated at the
 *     final gate).
 *   - Advancing semantic rules are owned entirely by TournamentSchema.
 *   - Winner unknown properties are preserved where schema permits.
 *   - Missing fields are represented as null/empty per schema
 *     canonicalisation.
 *
 * DEPENDENCIES:
...
(function() {
    ...
    var CalendarValidation = window.CalendarValidation || null;
    ...
    if (!Schema) {
        ...
        if (typeof Schema.validateTournament !== 'function') {
            _missing.push('TournamentSchema.validateTournament');
        }
    ...
    if (_missing.length > 0) {
        throw new Error(
            '[TournamentsRepair] Missing mandatory dependencies: ' +
            _missing.join(', ')
        );
    }
    ...
    function deriveLoser(participants, winner) {
        if (typeof Schema.deriveLoser === 'function') {
    ...
    function deriveAdvancing(match) {
        if (typeof Schema.deriveAdvancing === 'function') {
        ...
        if (match.type === 'group_exam') {
            ...
            for (var i = 0; i < participants.length; i++) {
                var id = normaliseId(participants[i]);
                if (id !== '' && match.results && match.results[id] === 'pass') {
                    advancing.push(id);
                }
            }
            return advancing;
    ...
    function isValidParticipantType(type) {
        if (typeof Schema.isValidParticipantType === 'function') {
    ...
    function isValidGroupExamResult(value) {
        if (typeof Schema.isValidGroupExamResult === 'function') {
    ...
    /**
     * Repair a single match.
     *
     * IDENTITY: the match's `id` is REQUIRED and PRESERVED. Repair
     * does not generate IDs; a match without a valid id is rejected
     * with a clear reason. This forces the operator to run the
     * schema's normaliseTournament (or the corresponding migration)
     * before repair, so identity is not silently rewritten.
     */
    function repairMatch(match, tournamentParticipants, expectedMatchSize, expectedMatchType, report) {
        ...
        if (matchId === '') {
            if (report) {
                report.discarded.push(
                    'Match has no valid id. Run normaliseTournament ' +
                    'before repair; repair does not generate IDs.'
                );
            }
            return null;
        }
        ...
        if (match.type === undefined) {
            if (report) {
                report.discarded.push('Match "' + matchId + '" has no type');
            }
            return null;
        }
        var type = match.type;
        if (!isValidMatchType(type)) {
            if (report) {
                report.discarded.push('Match "' + matchId + '" has invalid type: "' + type + '"');
            }
            return null;
        }
        ...
        if (expectedMatchType && type !== expectedMatchType) {
            if (report) {
                report.discarded.push('Match "' + matchId + '" type "' + type +
        ...
        if (match.status === undefined) {
            if (report) {
                report.discarded.push('Match "' + matchId + '" has no status');
            }
            return null;
        }
        ...
        if (!isValidMatchStatus(status)) {
            if (report) {
                report.discarded.push('Match "' + matchId + '" has invalid status: "' + status + '"');
            }
            return null;
        }
        ...
        if (!Array.isArray(match.participants)) {
            if (report) {
                report.discarded.push('Match "' + matchId + '" participants is not an array');
            }
            return null;
        }
        ...
        if (match.participants.length < 2) {
            if (report) {
                report.discarded.push('Match "' + matchId + '" has fewer than 2 participants');
            }
            return null;
        }
        ...
        for (var i = 0; i < match.participants.length; i++) {
            ...
            if (normalised === '') {
                if (report) {
                    report.discarded.push('Match "' + matchId +
                        '" has invalid participant ID: "' + String(id) + '"');
                }
                return null;
            }
            ...
            if (seen[normalised] === true) {
                if (report) {
                    report.discarded.push('Match "' + matchId +
                        '" has duplicate participant: "' + normalised + '"');
                }
                return null;
            }
            ...
            if (!participantExists(tournamentParticipants, normalised)) {
                if (report) {
                    report.discarded.push('Match "' + matchId +
                        '" participant "' + normalised + '" not found in tournament');
                }
                return null;
            }
        ...
        if (validParticipants.length !== expectedMatchSize) {
            if (report) {
                report.discarded.push('Match "' + matchId +
                    '" participant count (' + validParticipants.length +
        ...
        if (match.winner !== undefined && match.winner !== null) {
            var winnerId = normaliseId(match.winner);
            if (winnerId === '' || validParticipants.indexOf(winnerId) === -1) {
                if (report) {
                    report.discarded.push('Match "' + matchId +
        ...
        if (match.loser !== undefined && match.loser !== null) {
            if (type === 'group_exam') {
                if (report) {
                    report.discarded.push('Match "' + matchId +
                        '" is a group exam and cannot have a loser field');
                }
                return null;
            }
            var loserId = normaliseId(match.loser);
            if (loserId === '' || validParticipants.indexOf(loserId) === -1) {
                if (report) {
                    report.discarded.push('Match "' + matchId +
                        '" loser is not a valid participant');
                }
                return null;
            }
            if (winner !== null && loserId === winner) {
                if (report) {
                    report.discarded.push('Match "' + matchId +
                        '" loser cannot be the same as winner');
                }
                return null;
            }
            loser = loserId;
        ...
        if (type === 'standard' && validParticipants.length === 2 && status === 'completed') {
            ...
            if (derivedLoser !== null) {
                if (loser === null) {
                    ...
                    if (report) {
                        report.discarded.push('Match "' + matchId +
                            '" loser conflicts with derived loser');
                    }
                    return null;
                }
            }
        ...
        if (match.advancing !== undefined) {
            ...
            for (var j = 0; j < match.advancing.length; j++) {
                ...
                if (advId === '' || validParticipants.indexOf(advId) === -1) {
                    if (report) {
                        report.discarded.push('Match "' + matchId +
                            '" advancing participant is not valid');
                    }
                    return null;
                }
                if (advancingSeen[advId] === true) {
                    if (report) {
                        report.discarded.push('Match "' + matchId +
                            '" has duplicate advancing participant: "' + advId + '"');
                    }
                    return null;
                }
        ...
        if (match.results !== undefined) {
            if (type !== 'group_exam') {
                if (report) {
                    report.discarded.push('Match "' + matchId +
                        '" has results but is not a group_exam');
                }
                return null;
            }
            ...
            for (var k = 0; k < resultKeys.length; k++) {
                ...
                var resultId = normaliseId(key);
                if (resultId === '' || validParticipants.indexOf(resultId) === -1) {
                    if (report) {
                        report.discarded.push('Match "' + matchId +
                            '" result participant "' + key + '" is not in the match');
                    }
                    return null;
                }
                if (resultSeen[resultId] === true) {
                    if (report) {
                        report.discarded.push('Match "' + matchId +
                            '" has duplicate result participant: "' + resultId + '"');
                    }
                    return null;
                }
                ...
                if (!isValidGroupExamResult(value)) {
                    if (report) {
                        report.discarded.push('Match "' + matchId +
                            '" result for "' + key + '" is invalid: "' + value + '"');
                    }
                    return null;
                }
                results[resultId] = value;
            }
        }
        ...
        if (status === 'completed') {
            if (type === 'standard' && !winner) {
                if (report) {
                    report.discarded.push('Match "' + matchId +
                        '" is a completed standard match with no winner');
                }
                return null;
            }
            if (type === 'group_exam') {
                for (var m = 0; m < validParticipants.length; m++) {
                    ...
                    if (!results[pid]) {
                        if (report) {
                            report.discarded.push('Match "' + matchId +
                                '" is a completed group exam missing a result for ' + pid);
                        }
                        return null;
                    }
                }
    ...
    /**
     * Repair a single round.
     *
     * IDENTITY: the round's `id` is REQUIRED and PRESERVED.
     * roundNumber is POSITIONAL per schema and recomputed from the
     * index; the stored value is ignored.
     */
    function repairRound(round, index, tournamentParticipants, report) {
        ...
        if (roundId === '') {
            if (report) {
                report.discarded.push(
                    'Round ' + (index + 1) + ' has no valid id. Run ' +
                    'normaliseTournament before repair; repair does not ' +
                    'generate IDs.'
                );
            }
            return null;
        ...
        if (round.status === undefined) {
            if (report) {
                report.discarded.push('Round "' + roundId + '" has no status');
            }
            return null;
        }
        ...
        if (!isValidMatchStatus(status)) {
            if (report) {
                report.discarded.push('Round "' + roundId +
        ...
        if (round.matchSize === undefined) {
            if (report) {
                report.discarded.push('Round "' + roundId + '" has no match size');
            }
            return null;
        }
        ...
        if (matchSize === null || matchSize < 2) {
            if (report) {
                report.discarded.push('Round "' + roundId +
                    '" has invalid match size: "' + round.matchSize + '"');
            }
            return null;
        }
        ...
        if (round.matchType === undefined) {
            if (report) {
                report.discarded.push('Round "' + roundId + '" has no match type');
            }
            return null;
        }
        ...
        if (!isValidMatchType(matchType)) {
            if (report) {
                report.discarded.push('Round "' + roundId +
                    '" has invalid match type: "' + matchType + '"');
            }
            return null;
        }
        ...
        if (round.matches === undefined) {
            if (report) {
                report.discarded.push('Round "' + roundId + '" has no matches array');
            }
            return null;
        }
        if (!Array.isArray(round.matches)) {
            if (report) {
                report.discarded.push('Round "' + roundId + '" matches is not an array');
            }
            return null;
        }
        ...
        for (var i = 0; i < round.matches.length; i++) {
            ...
            if (repairedMatch !== null) {
                if (seenMatchIds[repairedMatch.id] === true) {
                    if (report) {
                        report.discarded.push('Round "' + roundId +
                            '" has duplicate match id: "' + repairedMatch.id + '"');
                    }
                    return null;
                }
                seenMatchIds[repairedMatch.id] = true;
                matches.push(repairedMatch);
            } else {
                if (matchReport.discarded.length > 0) {
                    matchReport.discarded.forEach(function(d) {
                        report.discarded.push(
                            'Round "' + roundId + '", Match ' + (i + 1) + ': ' + d
                        );
                    });
                }
                report.discarded.push('Round "' + roundId +
                    '" contains an unrecoverable match');
                return null;
            }
        }
    ...
    function repairElimination(elimination, tournamentParticipants, startWeek, endWeek, report) {
        ...
        if (!participant) {
            if (report) {
                report.discarded.push('Eliminated participant "' + participantId +
                    '" not found in tournament');
            }
            return null;
        }
        ...
        if (!isValidParticipantType(participantType)) {
            if (report) {
                report.discarded.push('Elimination has invalid participant type: "' +
                    participantType + '"');
            }
            return null;
        }
        ...
        if (participant.type !== participantType) {
            if (report) {
                report.discarded.push('Elimination participant type "' +
                    participantType + '" does not match tournament ' +
        ...
        if (week === null) {
            if (report) {
                report.discarded.push('Elimination has invalid week: "' +
            ...
            if (week < startWeek || week > endWeek) {
                if (report) {
                    report.discarded.push('Elimination week ' + week +
                        ' is outside tournament week range ' +
                        startWeek + '-' + endWeek);
                }
                return null;
            }
    ...
    var TournamentsRepair = {
         ...
         * Does NOT auto-save; caller is responsible for persistence.
         *
         * REJECTS (does not repair) any tournament whose rounds or
         * matches lack stable IDs. Run Schema.normaliseTournament
         * before calling repair when the input may lack IDs.
         *
         * @param {object} tourn - Tournament object to repair
         * @param {object} options - Reserved for future use
         * @returns {object|null} { tournament, report } or null
         */
        repairTournament: function(tourn, options) {
            ...
            if (startWeek > endWeek) {
                report.discarded.push('Start week (' + startWeek +
                    ') is after end week (' + endWeek + ')');
                return null;
            }
            ...
            if (tourn.createdAt !== undefined && tourn.createdAt !== null) {
                if (!isStrictDate(tourn.createdAt)) {
                    report.discarded.push('createdAt is not a valid ISO 8601 date');
                    return null;
                }
            ...
            if (tourn.graduatingClassId !== undefined &&
                tourn.graduatingClassId !== null) {
            ...
            for (var r = 0; r < tourn.rounds.length; r++) {
                ...
                var repairedRound = repairRound(
                    tourn.rounds[r], r, tournamentParticipants, roundReport
                );
                if (repairedRound !== null) {
                    if (seenRoundIds[repairedRound.id] === true) {
                        report.discarded.push('Duplicate round id: "' +
                            repairedRound.id + '"');
                        return null;
                    }
                    seenRoundIds[repairedRound.id] = true;
            ...
            for (var e = 0; e < tourn.eliminations.length; e++) {
                ...
                if (repairedE !== null) {
                    if (seenEliminations[repairedE.participantId] === true) {
                        report.discarded.push('Duplicate elimination for participant: "' +
                            repairedE.participantId + '"');
                        return null;
                    }
            ...
            if (tourn.winner !== undefined && tourn.winner !== null) {
                if (typeof tourn.winner === 'object' && tourn.winner !== null &&
                    ...
                    if (!participant) {
                        report.discarded.push('Winner "' + winnerId +
                            '" is not a tournament participant');
                        return null;
                    }
                    if (participant.type !== winnerType) {
                        report.discarded.push('Winner type "' + winnerType +
                            '" does not match participant type "' +
                            participant.type + '"');
                        return null;
                    }
                    var winnerEliminated = repaired.eliminations.some(function(elim) {
                    ...
                    // Winner is just an ID.
                    var bareWinnerId = normaliseId(tourn.winner);
                    if (bareWinnerId === '') {
                        report.discarded.push('Winner has invalid ID');
                        return null;
                    }
                    var bareParticipant = getParticipant(
                        tournamentParticipants, bareWinnerId
                    );
                    if (!bareParticipant) {
                        report.discarded.push('Winner "' + bareWinnerId +
                            '" cannot be resolved to a tournament participant');
                        return null;
                    }
                    var bareEliminated = repaired.eliminations.some(function(elim) {
                        return elim.participantId === bareWinnerId;
                    });
                    if (bareEliminated) {
                        report.discarded.push('Winner "' + bareWinnerId +
                            '" has been eliminated');
                    ...
                    repaired.winner = {
                        id: bareParticipant.id,
                        type: bareParticipant.type
                    };
                }
            }
            ...
            var knownTopKeys = [
                ...
                'graduatingClassId', 'classFilterEnabled', 'createdAt',
                '_schemaVersion'
            ];
            ...
            if (!finalValidation.valid) {
                finalValidation.errors.forEach(function(error) {
                    report.discarded.push(
                        'Repaired tournament failed strict validation: ' + error
                    );
                });
                return null;
            }
         ...
         *
         * ATOMIC in-memory: either all tournaments are repaired, or
         * none are mutated. Returns a detailed repair report with all
         * nested failure reasons preserved.
         */
